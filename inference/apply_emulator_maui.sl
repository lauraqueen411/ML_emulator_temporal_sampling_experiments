...
/nesi/project/niwa00018/rampaln/envs/ml_env_v2/bin/python run_emulator_temporal_tests.py $gan $variable $input_dir $gcm $ssp $variant $output_dir $ml_model $code_dir "imperfect" $epoch

# apply to historical+ssp period in perfect framework
/nesi/project/niwa00018/rampaln/envs/ml_env_v2/bin/python run_emulator_temporal_tests.py $gan $variable $input_dir $gcm $ssp $variant $output_dir $ml_model $code_dir "perfect" $epoch

# compute metrics
/nesi/project/niwa00018/rampaln/envs/ml_env_v2/bin/python compute_metrics.py $ml_model $gcm $variable $ssp $output_dir $gan $epoch