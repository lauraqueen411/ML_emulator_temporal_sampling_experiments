...
def load_emulator_data(GCM,var,ssp,ml_model,ml_output_dir,epoch):

    # Emulator - imperfect
    imperfect_hist = xr.open_dataset(f'{ml_output_dir}/{GCM}/{ml_model}/{GCM}_{var}_historical_imperfect_framework_{gan_flag}_epoch_{str(epoch)}.nc')[var]
    imperfect_future = xr.open_dataset(f'{ml_output_dir}/{GCM}/{ml_model}/{GCM}_{var}_{ssp}_imperfect_framework_{gan_flag}_epoch_{str(epoch)}.nc')[var]
    imperfect_da = xr.concat([imperfect_hist,imperfect_future],dim='time')

    # Emulator - perfect
    perfect_da = xr.open_dataset(f'{ml_output_dir}/{GCM}/{ml_model}/{GCM}_{var}_{ssp}_perfect_framework_{gan_flag}_epoch_{str(epoch)}.nc')[var]
...
def compute_save_emulator(GCM,var,emulator,ml_output_dir,epoch):
    
    print('\t\tloading emulator data: ' + GCM + ' ' + var + ' (' + emulator + ')')
    try:
        da_dict = load_emulator_data(GCM,var,ssp,emulator,ml_output_dir,epoch)
    except:
    ...
    for framework in da_dict:
        ...
        metric_ds.to_netcdf(f'{ml_output_dir}/metrics/{GCM}/{emulator}/{GCM}_{framework}_{var}_metrics_{gan_flag}_epoch_{str(epoch)}.nc')
...
if not (os.path.exists(f'{ml_output_dir}/metrics/{gcm}/{emulator}/{gcm}_imperfect_{var}_metrics_{gan_flag}_epoch_{str(epoch)}.nc') and os.path.exists(f'{ml_output_dir}/metrics/{gcm}/{emulator}/{gcm}_perfect_{var}_metrics_{gan_flag}_epoch_{str(epoch)}.nc')):
    compute_save_emulator(gcm,var,emulator,ml_output_dir,epoch)
