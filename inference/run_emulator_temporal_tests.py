...
if os.path.exists(f'{output_dir}/{gcm}/{ml_model_name}/{gcm}_{variable}_{ssp}_{framework}_framework_{gan_flag}_epoch_{str(epoch)}.nc'):
...
config_file = r'/nesi/project/niwa00018/queenle/ML_emulator_temporal_sampling_experiments/training/emulators/' + ml_model_name + '/config_info.json'
...
# LOAD ML MODEL
print('LOAD ML MODEL\n')
gan, unet, adv_factor = load_model_cascade(ml_model_name, epoch, '/nesi/project/niwa00018/queenle/ML_emulator_temporal_sampling_experiments/training/emulators', load_unet=True)
...
output.to_netcdf(f'{output_dir}/{gcm}/{ml_model_name}/{gcm}_{variable}_{ssp}_{framework}_framework_{gan_flag}_epoch_{str(epoch)}.nc')
print('\n\n')
