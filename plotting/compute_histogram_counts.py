...
result_dir = '/nesi/project/niwa00018/queenle/ML_emulator_temporal_sampling_experiments/plotting/histogram_counts'
...
def get_ml_counts(n,gcm,ml_type,framework,epoch,start,end,bins):
    ...
    if framework == 'imperfect' and int(start) < 2015:
        da = xr.open_dataarray(f'{ml_output_dir}/{gcm}/pr_ACCESS-CM2_{str(n)}/{gcm}_pr_historical_{framework}_framework_{ml_type}_epoch_{epoch}.nc')
    else:
        da = xr.open_dataarray(f'{ml_output_dir}/{gcm}/pr_ACCESS-CM2_{str(n)}/{gcm}_pr_ssp370_{framework}_framework_{ml_type}_epoch_{epoch}.nc')
