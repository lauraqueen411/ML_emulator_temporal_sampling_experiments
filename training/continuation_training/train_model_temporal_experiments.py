...
with strategy.scope():
    ...
    generator = tf.keras.models.load_model(f'{config["output_folder"]}/{config["model_name"]}/generator_epoch_200.h5',
    ...
    unet_model = tf.keras.models.load_model(f'{config["output_folder"]}/{config["model_name"]}/unet_epoch_200.h5',
                                          custom_objects={"BicubicUpSampling2D": BicubicUpSampling2D,
                                                          "SymmetricPadding2D": SymmetricPadding2D}, compile=False)

    
    #d_model = get_discriminator_model_v1(tuple(output_shape) + (n_output_channels,), tuple(input_shape) + (n_channels,))
    
    d_model = tf.keras.models.load_model(f'{config["output_folder"]}/{config["model_name"]}/discriminator_epoch_200.h5',
                                          custom_objects={"BicubicUpSampling2D": BicubicUpSampling2D,
                                                          "SymmetricPadding2D": SymmetricPadding2D}, compile=False)
